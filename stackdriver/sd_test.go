...
func Test_FormatterOptions(t *testing.T) {
	...
	xlog.SetFormatter(NewFormatter(writer, "sd").
		Options(xlog.FormatWithCaller(true), xlog.FormatWithLocation(true), xlog.FormatSkipTime(true), xlog.FormatPrintEmpty(true)))

	logger.KV(xlog.INFO, "k1", 1, "k2", false, "nil", nil, "empty", "")
	result := b.String()
	assert.Equal(t, `{"logName":"sd","component":"stackdriver","message":{"k1":1,"k2":false,"nil":null},"severity":"INFO","sourceLocation":{"file":"sd_test.go","line":25,"function":"Test_FormatterOptions"}}`+"\n", result)
	b.Reset()

	xlog.SetFormatter(NewFormatter(writer, "sd").
		Options(xlog.FormatWithCaller(false), xlog.FormatWithLocation(true), xlog.FormatSkipTime(true), xlog.FormatPrintEmpty(true)))

	logger.KV(xlog.INFO, "k1", 1, "k2", false, "nil", nil, "empty", "")
	result = b.String()
	assert.Equal(t, `{"logName":"sd","component":"stackdriver","message":{"k1":1,"k2":false,"nil":null},"severity":"INFO","sourceLocation":{"function":"Test_FormatterOptions"}}`+"\n", result)
	b.Reset()

	xlog.SetFormatter(NewFormatter(writer, "sd").
		Options(xlog.FormatWithCaller(false), xlog.FormatWithLocation(true), xlog.FormatSkipTime(true)))
...
func Test_Formatter(t *testing.T) {
	...
	xlog.SetGlobalLogLevel(xlog.INFO)
	xlog.SetFormatter(NewFormatter(writer, "sd").Options(xlog.FormatWithCaller(true)))
